#!/bin/bash
# Finds all WordPress installations under a given directory (default: current dir),
# updates each to the latest core, plugins, themes, and language files, and
# configures Postmark for outbound email. Requires ~/.postmark_api_key to be set.

if [[ ! -f ~/.postmark_api_key ]]; then
...
function updatePostmarkSettings {
	...
	if [[ $(wp config has MULTISITE && wp config get MULTISITE) == 1 ]]; then
		sudo -u "$OWNER" wp site list --field=url --archived=false | while read -r URL; do
...
find -L . -name wp-settings.php -not -regex ".*vault.*" -not -regex ".*archive.*" -exec realpath "{}" \; 2>/dev/null | sort | uniq | while read -r linein; do
	DIR=$(dirname "$linein")
